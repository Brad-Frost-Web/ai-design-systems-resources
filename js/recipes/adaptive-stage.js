		...
		return html`<ed-accordion-panel>
			...
			<ed-text-passage size="sm">
				<p>
					...
					guidelines (${shortDate(cat.generated)}). The design system's documented DOs and DON'Ts are what
					the engine chooses from — the same guidance a human would read, made machine-readable.
				</p>
			</ed-text-passage>
			<ed-table variant="zebra" fullWidth label="Component catalog">
				<ed-table-header>
					<ed-table-row>
						<ed-table-header-cell>Shape</ed-table-header-cell>
						...
						<ed-table-header-cell>Eddie says: do</ed-table-header-cell>
						<ed-table-header-cell>Eddie says: don't</ed-table-header-cell>
					</ed-table-row>
				</ed-table-header>
				<ed-table-body>
					${cat.nodes.map(
						(n) => html`<ed-table-row>
							...
							<ed-table-cell>${n.eddie?.use?.[0] ? clip(n.eddie.use[0], 140) : "—"}</ed-table-cell>
							<ed-table-cell>${n.eddie?.dontUse?.[0] ? clip(n.eddie.dontUse[0], 140) : "—"}</ed-table-cell>
						</ed-table-row>`,
					)}
				</ed-table-body>
			</ed-table>
